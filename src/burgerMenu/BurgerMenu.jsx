...
export default function TemporaryDrawer({ isScrolled }) {
  ...
  const handleScrollToAtservices = (text) => {
    ...
    setTimeout(() => {
      const element = document.getElementById(anchor);
      const validIds = ["nosobjectifs", "nosvaleurs", "nosmoyenshumains"];
      console.log("element", element);

      if (validIds.includes(element.id) && windowsWidth <= 1000) {
        navigate("/AboutATservices");
      }
      if (element) {
        // Calculer la position de défilement ajustée pour faire défiler 100 pixels au-dessus de l'élément ciblé
        const { top } = element.getBoundingClientRect();
  ...
  return (
    <div>
      <Button onClick={toggleDrawer(true)}>
        <img
          src={isScrolled ? WhiteBurgerMenu : Burger}
          alt=""
